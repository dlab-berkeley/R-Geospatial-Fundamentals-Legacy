...
there's a function for that too (which we already saw in Part I).
...
Those colors were controlled by a bunch of information in the original
NLCD file, which was read into the `@legend` slot of our raster object.
Here's the info (with the commonly used hexadecimal format for color codes, e.g. '#00F900'):
...
Of course, these are __not statistically valid__ models! Just analytical stand-ins.
