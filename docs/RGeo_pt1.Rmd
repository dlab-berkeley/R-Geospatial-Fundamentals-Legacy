...
<img src="images/sticky_geometries.jpg" width="400px"></img>
...
Now, how do we manipulate more of the aspects in our plots?

How do we make our plots prettier?

And what other options do we have for plotting these explicity geospatial `sf` objects?
...
If so, what does that tell us about `tmap`?
...
And we can use `tmap_save` to save maps to files.
...
```{r, eval=F}
tmap_save(map1, "./output/SF_properties.png", height=6) # Static image file with
tmap_save(map1, "./output/SF_properties.html") # interactive web map
```
