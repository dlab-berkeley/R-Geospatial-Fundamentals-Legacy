...
Alternately, you can use the `here()` function from the `here` package to build file paths that start at the project directory.
...
SFtracts <- st_read(here("notebook_data",
                         "part_3", 
                         "sftracts_wpop"))
```
...
SFhomes15_sf <- st_as_sf(SFhomes15, 
                         coords = c('lon', 'lat'),
                         crs = 4326)
...
But it still left us with values outside of the census tracts themselves (because they are areas outside
...
writeRaster(DEM_WGS_crop_masked, 
...
#reproject
nlcd_WGS = projectRaster(nlcd, 
                         projectExtent(nlcd, st_crs(SFtracts)$proj4string))
