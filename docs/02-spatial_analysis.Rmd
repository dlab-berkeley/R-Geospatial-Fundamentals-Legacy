...
And note that `st_geometry(sf_obj)` does the same thing as `sf_obj$geometry`.

```{r}
plot(SFboundary_utm$geometry)
plot(SFhighways$geometry, col='purple', lwd=4, add = T)
plot(SFhomes15_utm$geometry, add = T, pch = 19, cex = 0.5)
plot(bart_utm$geometry, col="skyblue", pch=19, cex = 1, add=T)
```
...
(Also notice the slight difference in our answers: Our UTM is not an equal-area projection. And for an unprojected CRS `sf` gives us *geodetic area* -- i.e. area on the ellipsoid.)
...
This leads to a very common type of spatial query called a `point-in-polygon` query.

We can use the `st_within` function to answer this.

We'll start with the simplest question: __*Are* there BART stations in SF?__

We already know the answer, but let's see how it's done.

## *Are* there BART stations in SF?
...
A **spatial join** (a.k.a. **spatial merge**) associates rows of data in one object with rows in another object based on the spatial relationship between the objects' geometries (in the same CRS).

<img src="./images/spatial_join.png" width="450px"></img>
...
1. using that output object to subset the `sftracts_utm` `data.frame`;
...
A standard attribute **join** (a.k.a. **merge**) associates rows of data with
rows in another object based on shared values in a column. 

<img src="./images/join.png" width="450px"></img>
...
We've pulled information from the census tracts 'down' to the homes.

Now let's pull information in the opposite direction, thinking about this question from the tract perspective. 
...
But we'll use `sf`'s **spatial aggregate** functionality instead.
...
- __`by`__: can be another `sf` object whose geometries will generate the groupings (among other possibilities)
...
```{r}
mean_totvalue_choropleth + tm_shape(SFhomes15_utm) + tm_dots(size = 0.01)
```
...
Now let's see an example of an analysis combining measurement-based and
...
## Properties within Walking Distance of BART

In order to select properties within walking distance of BART, we can:

1. Define a 'walking distance' (say, 1 km);

1. Create a 1 km-radius buffer polygon around each BART point;

2. Do a point-in-polygon operation to either find the properties within that distance.

## Create the buffers 

To create the buffers we'll use---surprise, suprise---`st_buffer`.
...
Important thought: **What happens to properties within 1 km of Daly City BART?**
