...
A CRS can be *projected* or *unprojected* (a.k.a. *geographic*).
...
```{r, echo=F}
knitr::include_url('https://xkcd.com/2256/#ctitle', height = "300px")
```
...
**Raster data**: Regular grid of cells (or pixels)
...
*All good? Are all layers displayed?*  
...
Our landmarks CRS appears totally different!
...
* increasingly supported by other geospatial and plotting packages (e.g. `tmap`, `ggplot`)
...
Note that we get an array of plots, one for each variable, (up to the first 9).
...
The latter is more machine-readable. The former is human-readable.

For example, here's what a WKT point would look like: `POINT (0 3)`

And a linestring: `LINESTRING (0 3, 1 4, 2 3)`
...
Or you can refer to a more technical depiction (from `sf`'s own [Github Repo](https://github.com/r-spatial/sf)):
...
**NOTE**: With the base `plot` function, we need to use `sf::st_geometry` to pull out just the sf object's geometric features, for plotting.
...
Here, we'll reproject the tracts,
