...
1. Open <https://github.com/dlab-berkeley/Geospatial-Fundamentals-in-R-with-sf>
    - Download & unzip the zip file
    - Make a note of the folder in which it is located

2. Start **RStudio** and open a new script file or today's .Rmd file.

3. Set your working directory to the repository you downloaded and unzipped.
...
```{r, eval=F}
our_packages<- c("ggplot2", "ggmap", "dplyr", "sf", "units", "tmap", "raster")
...
## A note on `ggmap` installation

If you have problems with `ggmap`, you may need to reinstall the library from github (using the code below).

(See this [StackOverflow discussion](https://stackoverflow.com/questions/34402979/increase-the-api-limit-in-ggmaps-geocode-function-in-r) for details.)
...
## A note on Google Maps authorization

`ggmap` requires you to have a registered Google Maps API key

This requires having a credit card on file.

For your own use you'll want to set this up privately (outside the scope of this workshop).

But for today, we have provided you a temporary API key (in `tmp_key.txt`), just for running this demo.

## Authorization

First, let's load and register our API key.

(The API key will be different in each workshop. See `tmp_key.txt`. But the code should look as follows.)

```{r, eval=F}
api_key = "AS24AHFAN322D3FN92NFN297FBNSLWU228RNN292F82NENDO9KSKW9"
...
##  get_map
Now let's check out the `get_map` function. This will allow us to fetch map tiles to display in our maps.
...
<img src="./images/shrugging_baby.jpg" width="600px"></img>
