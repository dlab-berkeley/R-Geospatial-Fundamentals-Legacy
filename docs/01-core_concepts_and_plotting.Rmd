...
## tmap: other point symbols

### CHALLENGE

Alter the previous map to try some other point map styles including:

- `tm_markers`, setting the mode to interactive

- `tm_bubbles`, setting the border color to red

See **?tm_symbols** for help if needed
...
Take a look at the documentation for tm_lines and then recreate the last map increasing the width of the lines
...
`tmap` uses the *classint* package to map colors to data values based on classification methods.

Let's take a look at the *style* argument in the documentation.
...
See how the choice of classification scheme, set by the *style* argument, alters the cartography.

```{r}
tmap_mode('plot')
jenks_map <- tm_shape(sfhomes15_sf) + 
  tm_dots(col="totvalue", size=.25, style="jenks")

quantile_map  <- tm_shape(sfhomes15_sf) + 
  tm_dots(col="totvalue", size=.25, style="quantile")
```

## `tmap`: color mapping

Compare the two classification schemes...

```{r}
tmap_arrange(jenks_map, quantile_map)
```
...
If time, try another basemap from the list
<http://leaflet-extras.github.io/leaflet-providers/preview>

```{r, eval=F}
tmap_mode('view')
#tm_basemap('<YOUR BASEMAP NAME HERE') + 
tm_shape(sfhomes15_sf) + 
tm_dots(col="totvalue", size=.05, title = "San Francisco Property Values (2015)") +
